...
(function() {
  ...
  Client.Mpeg = (function(_super) {
    ...
    Mpeg.prototype._transform = function(chunk, encoding, callback) {
      var after, afterLen, before, data, remaining;
      data = chunk.data;
      switch (chunk.type) {
        case "metadata":
          this.metadata = data;
        ...
        case "data":
          ...
          while (this.byteCount + data.length > this.icyMetadataInterval) {
            before = data.slice(0, this.icyMetadataInterval - this.byteCount);
            remaining = data.slice(this.icyMetadataInterval - this.byteCount);
            afterLen = Math.min(this.icyMetadataInterval, remaining.length);
            after = remaining.slice(0, afterLen);
            data = remaining.slice(afterLen);
            this.byteCount = afterLen;
